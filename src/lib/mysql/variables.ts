...
export class VariablesDB {
	...
	static async set(name: string, value: string | number) {
		const res = await sendSQLQuery("INSERT INTO variables (name, value) VALUES (?, ?) ON DUPLICATE KEY UPDATE value = ?;", [name, value, value]);
	...
	static async get(varName: string | number) {
		const res = await sendSQLQuery("SELECT * FROM variables WHERE name = ?;", [varName]);
	...
	static async getAll() {
		const res = await sendSQLQuery("SELECT * FROM variables;");
