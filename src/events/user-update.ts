...
export async function handleUserUpdate(config: Config, oldUser: User | PartialUser, newUser: User) {
	const logChannel = await newUser.client.channels.fetch(config.log.userlogChannelId);
	...
	if (oldUser.avatar !== newUser.avatar) {
		...
		const oldName = `${oldUser.username}#${oldUser.discriminator}`;
		const newName = `${newUser.username}#${newUser.discriminator}`;
